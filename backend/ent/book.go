...
type Book struct {
	...
	// Recom holds the value of the "recom" field.
	Recom string `json:"recom,omitempty"`
...
func (*Book) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case book.FieldID, book.FieldUserID, book.FieldPrice:
			values[i] = new(sql.NullInt64)
		case book.FieldTitle, book.FieldKind, book.FieldRecom:
...
func (b *Book) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case book.FieldRecom:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field recom", values[i])
			} else if value.Valid {
				b.Recom = value.String
			}
		default:
...
func (b *Book) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("recom=")
	builder.WriteString(b.Recom)
	builder.WriteByte(')')
