...
// The MissFunc type is an adapter to allow the use of ordinary
// function as Miss mutator.
type MissFunc func(context.Context, *ent.MissMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f MissFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.MissMutation); ok {
		return f(ctx, mv)
	}
