...
func (uc *UserCreate) AddMissIDs(ids ...int) *UserCreate {
	uc.mutation.AddMissIDs(ids...)
	return uc
}

// AddMisses adds the "misses" edges to the Miss entity.
func (uc *UserCreate) AddMisses(m ...*Miss) *UserCreate {
	ids := make([]int, len(m))
	for i := range m {
		ids[i] = m[i].ID
	}
	return uc.AddMissIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.MissesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
