...
func (tc *TokenCreate) SetAccessDate(t time.Time) *TokenCreate {
	tc.mutation.SetAccessDate(t)
	return tc
}

// SetNillableAccessDate sets the "access_date" field if the given value is not nil.
func (tc *TokenCreate) SetNillableAccessDate(t *time.Time) *TokenCreate {
	if t != nil {
		tc.SetAccessDate(*t)
	}
	return tc
...
func (tc *TokenCreate) defaults() {
	if _, ok := tc.mutation.AccessDate(); !ok {
		v := token.DefaultAccessDate()
		tc.mutation.SetAccessDate(v)
	}
}
...
func (tc *TokenCreate) check() error {
	...
	if _, ok := tc.mutation.AccessDate(); !ok {
		return &ValidationError{Name: "access_date", err: errors.New(`ent: missing required field "Token.access_date"`)}
	}
	if _, ok := tc.mutation.UserID(); !ok {
...
func (tc *TokenCreate) createSpec() (*Token, *sqlgraph.CreateSpec) {
	...
	if value, ok := tc.mutation.AccessDate(); ok {
		_spec.SetField(token.FieldAccessDate, field.TypeTime, value)
		_node.AccessDate = value
	}
