...
func (lq *LockQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Lock, error) {
	...
	if query := lq.withUser; query != nil {
		if err := lq.loadUser(ctx, query, nodes, nil,
			func(n *Lock, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (lq *LockQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Lock, init func(*Lock), assign func(*Lock, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Lock)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (lq *LockQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := lq.ctx.Fields; len(fields) > 0 {
		...
		if lq.withUser != nil {
			_spec.Node.AddColumnOnce(lock.FieldUserID)
		}
	}
