...
func (bc *BookCreate) SetKind(s string) *BookCreate {
	bc.mutation.SetKind(s)
	return bc
}
...
func (bc *BookCreate) SetRecom(s string) *BookCreate {
	bc.mutation.SetRecom(s)
	return bc
}
...
func (bc *BookCreate) check() error {
	...
	if _, ok := bc.mutation.Recom(); !ok {
		return &ValidationError{Name: "recom", err: errors.New(`ent: missing required field "Book.recom"`)}
	}
	if _, ok := bc.mutation.UserID(); !ok {
...
func (bc *BookCreate) createSpec() (*Book, *sqlgraph.CreateSpec) {
	...
	if value, ok := bc.mutation.Kind(); ok {
	...
	if value, ok := bc.mutation.Recom(); ok {
