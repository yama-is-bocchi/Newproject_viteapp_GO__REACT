...
// QueryMisses chains the current query on the "misses" edge.
func (uq *UserQuery) QueryMisses() *MissQuery {
	query := (&MissClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryLocks() *LockQuery {
	query := (&LockClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryWantlists chains the current query on the "wantlists" edge.
func (uq *UserQuery) QueryWantlists() *WantlistQuery {
	query := (&WantlistClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withWantlists: uq.withWantlists.Clone(),
...
// WithMisses tells the query-builder to eager-load the nodes that are connected to
// the "misses" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithMisses(opts ...func(*MissQuery)) *UserQuery {
	query := (&MissClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithWantlists tells the query-builder to eager-load the nodes that are connected to
// the "wantlists" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithWantlists(opts ...func(*WantlistQuery)) *UserQuery {
	query := (&WantlistClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withWantlists = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withMisses; query != nil {
		if err := uq.loadMisses(ctx, query, nodes,
			func(n *User) { n.Edges.Misses = []*Miss{} },
			func(n *User, e *Miss) { n.Edges.Misses = append(n.Edges.Misses, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withLocks; query != nil {
		if err := uq.loadLocks(ctx, query, nodes,
			func(n *User) { n.Edges.Locks = []*Lock{} },
			func(n *User, e *Lock) { n.Edges.Locks = append(n.Edges.Locks, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := uq.withWantlists; query != nil {
		if err := uq.loadWantlists(ctx, query, nodes,
			func(n *User) { n.Edges.Wantlists = []*Wantlist{} },
			func(n *User, e *Wantlist) { n.Edges.Wantlists = append(n.Edges.Wantlists, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadMisses(ctx context.Context, query *MissQuery, nodes []*User, init func(*User), assign func(*User, *Miss)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(miss.FieldUserID)
	}
	query.Where(predicate.Miss(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.MissesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (uq *UserQuery) loadLocks(ctx context.Context, query *LockQuery, nodes []*User, init func(*User), assign func(*User, *Lock)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(lock.FieldUserID)
	}
	query.Where(predicate.Lock(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.LocksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (uq *UserQuery) loadWantlists(ctx context.Context, query *WantlistQuery, nodes []*User, init func(*User), assign func(*User, *Wantlist)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(wantlist.FieldUserID)
	}
	query.Where(predicate.Wantlist(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.WantlistsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
