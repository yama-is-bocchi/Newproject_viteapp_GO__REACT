...
type Token struct {
	...
	// AccessDate holds the value of the "access_date" field.
	...
	for i := range columns {
		...
		case token.FieldAccessDate:
			if value, ok := values[i].(*sql.NullTime); !ok {
				return fmt.Errorf("unexpected type %T for field access_date", values[i])
			} else if value.Valid {
				t.AccessDate = value.Time
			}
		default:
...
func (t *Token) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("access_date=")
	builder.WriteString(t.AccessDate.Format(time.ANSIC))
	builder.WriteByte(')')
